...
public final class XmpMetadata {
     ...
     * <code>propertyInfos</code> matching that namespace
     */
     ...
     * @return Metadata or null if the sidecar file is null or on errors while reading
     * @throws IOException
     */
     ...
     * @param toXmpPropertyInfos list of property infos to retrieve the properties from XMP metadata
     */
    ...
    /**
     * Returns whether a sidecar file can be written to the file system. <p> This is true if the sidecar file exists and
     * is writable or if it does not exist and the directory is writable.
     *
     * @param imageFile image file or null
    ...
    /**
     * Writes the values of a {@code Xmp} instance as or into a XMP sidecar file.
     *
    ...
    /**
     * Deletes from an
     * <code>XMPMeta</code> instance all data an user can edit.
     *
     * @param toXmpMeta XMP metadata
     */
     ...
     * @param fromXmp
     * <code>Xmp</code> metadata to set from
     * @param toXmpMeta
     * <code>XMPMeta</code> metadata to set to
     * @param namespaceUri URI of namespase in
     * <code>XMPMeta</code> to set
    ...
    /**
     * Returns XMP metadata of a image file. <p> If the image has a sidecar file, it's metadata will be read. If the
     * image hasn't a sidecar file but embedded XMP metadata, the embedded XMP metadata will be read.
     *
     * @param imageFile image file or null
    ...
    public static Xmp getXmpFromSidecarFileOf(File imageFile) throws IOException {
        ...
        File sidecarFile = XMP_SIDECAR_FILE_RESOLVER.getXmpSidecarFileOrNullIfNotExists(imageFile);
        List<XMPPropertyInfo> propertyInfosOfSidecarFile = getPropertyInfosOfSidecarFile(sidecarFile);
        return getXmp(propertyInfosOfSidecarFile, imageFile, XmpLocation.SIDECAR_FILE);
    }
     ...
     * @return embedded XMP metadata or null if no XMP is embedded or when errors occur while reading the file
     */
     ...
     *                    {@code ImageFileSidecarFile#getDefaultXmpSidecarFileOrNullIfNotExists()} returns null if the image file has no sidecar
     * file.
     */
