 ...
 */
public class ExifTagTest {

    @Test
    public void testWriteToFile() throws Exception {
        File file = getFile();
        try {
            ExifTag exifTag1 = new ExifTag(125, 250, 500, 1000L, new byte[]{0x3C, 0x78, 0x3A}, "abc", 47, "bla", ExifIfd.EXIF);
            ExifTag exifTag2 = new ExifTag(521, 52, 5, 1L, new byte[]{0x30, 0x75, 0x0A}, "def", 74, "blu", ExifIfd.GPS);
    ...
    private void assertExifTagsEquals(ExifTag exifTag1, ExifTag exifTag2) {
        ...
        assertEquals(exifTag1.parseValueType(), exifTag2.parseValueType());
        assertEquals(exifTag1.parseProperties(), exifTag2.parseProperties());
        assertEquals(exifTag1.getTagId(), exifTag2.getTagId());
        assertEquals(exifTag1.getIfd(), exifTag2.getIfd());
