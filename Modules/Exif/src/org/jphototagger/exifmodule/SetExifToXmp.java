...
public final class SetExifToXmp extends HelperThread {
    ...
    private static final ImageFilesRepository imageFilesRepo = Lookup.getDefault().lookup(ImageFilesRepository.class);
    ...
    public void run() {
        List<File> imgFiles = (files == null)
                ? imageFilesRepo.findAllImageFiles()
                : files;
        ...
        if ((exif != null) && exifHasValues(exif)) {
            if (isSet(xmp, replaceExistingXmpData)) {
                ...
                if (XmpMetadata.writeXmpToSidecarFile(xmp, xmpFile)) {
                    ...
                    imageFilesRepo.saveOrUpdateImageFile(imageFile);
                }
            }
        }
    }
