...
import de.elmar_baumann.jpt.exporter.HierarchicalKeywordsExporterJpt;
...
/**
 * Imports Keywords exported by {@link HierarchicalKeywordsExporterJpt}.
 *
 * @author  Elmar Baumann <eb@elmar-baumann.de>
 * @version 2009-10-11
 */
public final class HierarchicalKeywordsImporterJpt
        implements HierarchicalKeywordsImporter, EntityResolver {

    public static final HierarchicalKeywordsImporterJpt INSTANCE =
            new HierarchicalKeywordsImporterJpt();
    ...
    public Collection<List<Pair<String, Boolean>>> getPaths(File file) {
        try {
            ...
            NodeList nl = doc.getElementsByTagName(
                    HierarchicalKeywordsExporterJpt.TAGNAME_ROOT);
            if (nl.getLength() > 0) {
                return getPaths(nl.item(0));
            }
        } catch (Exception ex) {
            AppLog.logSevere(getClass(), ex);
            MessageDisplayer.error(null, "HierarchicalKeywordsImporterJpt.Error");
        }
        return null;
    }
    ...
    private Pair<String, Boolean> getKeyword(Node node) {
        ...
        String name = attr.getNamedItem(
                HierarchicalKeywordsExporterJpt.ATTRIBUTE_NAME).getNodeValue();
        Boolean real =
                attr.getNamedItem(
                HierarchicalKeywordsExporterJpt.ATTRIBUTE_TYPE).getNodeValue().
                equals(
                HierarchicalKeywordsExporterJpt.VALUE_OF_ATTRIBUTE_TYPE.get(true));
    ...
    private void addLeaf(Collection<Node> leafs, Node node) {
        if (!node.getNodeName().equals(
                HierarchicalKeywordsExporterJpt.TAGNAME_KEYWORD))
            return;
    ...
    public FileFilter getFileFilter() {
        return HierarchicalKeywordsExporterJpt.FILE_FILTER;
    }

    @Override
    public String getDescription() {
        return HierarchicalKeywordsExporterJpt.DESCRIPTION;
    }

    @Override
    public Icon getIcon() {
        return HierarchicalKeywordsExporterJpt.ICON;
    }

    private HierarchicalKeywordsImporterJpt() {
    }
