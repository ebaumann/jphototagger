...
public final class ExifMakerNote {
    ...
    public void addMakerNotes(Collection<IfdEntryProxy> entries) {

        List<IfdEntryProxy> mnEntries = ExifMetadata.getMakerNodeEntries(entries, byteOffsetToIfd);

        if (mnEntries != null) {
            try {
            add(mnEntries, entries);
            } catch (Exception ex) {
    ...
    private void add(List<IfdEntryProxy> from, Collection<IfdEntryProxy> to) throws NoSuchMethodException, InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException {
    ...
    private void add(
            List<IfdEntryProxy>       from,
            Collection<IfdEntryProxy> to,
            List<MakerTagInfo>          infos) throws NoSuchMethodException, InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException {

        int index = 0;
        for (IfdEntryProxy entry : from) {
            ...
            if (pos >= 0) {
                ...
                if  (requiresByteOrder(dataTypeClass)) {
                    Constructor c = dataTypeClass.getConstructor(byte[].class, ExifByteOrder.class);
                    dataType = c.newInstance(rV, entry.byteOrder());
                } else {
                    Constructor c = dataTypeClass.getConstructor(byte[].class);
                    dataType = c.newInstance(rV);
                }

                IfdEntryProxy ep = new IfdEntryProxy(
                        info.equalsToExifTag() ? info.equalsToExifTagId : ExifTag.MAKER_NOTE.tagId(),
                        entry.type(),
                        rV,
                        dataType.toString(),
                        bundle.getString(info.promptPropertyKey()),
                        entry.byteOrder(),
                        entry.byteOrderValue());
                ep.setFormatterClass(info.exifFormatterClass);
                to.add(ep);
            }
        }
    }
