...
public final class ExifGpsUtil {
        ...
        if ((dateStamp != null) && (timeStamp != null) && dateStamp.isValid()) {
            try {
                ...
                cal.set(Calendar.HOUR_OF_DAY,
                        (int) ExifDatatypeUtil.toDouble(timeStamp.hours()));
                cal.set(Calendar.MINUTE,
                        (int) ExifDatatypeUtil.toDouble(timeStamp.minutes()));
                cal.set(Calendar.SECOND,
                        (int) ExifDatatypeUtil.toDouble(timeStamp.seconds()));
    ...
    private static void setGpsDate(ExifGpsMetadata gpsMetaData,
                                   ExifTags exifTags) {
        ExifTag tagDate =
            exifTags.gpsTagById(ExifTag.Id.GPS_DATE_STAMP.value());
    ...
    private static void setGpsTime(ExifGpsMetadata gpsMetaData,
                                   ExifTags exifTags) {
        ExifTag tagTime =
            exifTags.gpsTagById(ExifTag.Id.GPS_TIME_STAMP.value());
        ...
        if (tagTime != null) {
            ...
            if ((rawValue != null) && (rawValue.length == 24)) {
                ...
                byte[]       minutesRawValue = Arrays.copyOfRange(rawValue, 8,
                                                   16);
                byte[]       secondsRawValue = Arrays.copyOfRange(rawValue, 16,
                                                   24);
                ExifRational hours = new ExifRational(hoursRawValue, byteOrder);
                ExifRational minutes = new ExifRational(minutesRawValue,
                                           byteOrder);
                ExifRational seconds = new ExifRational(secondsRawValue,
                                           byteOrder);
                ExifGpsTimeStamp timeStamp = new ExifGpsTimeStamp(hours,
                                                 minutes, seconds);
