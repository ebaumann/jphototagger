...
public final class DatabaseTables extends Database {
    ...
    private void createFilesTable(Connection con, Statement stmt) throws SQLException {
        if (!DatabaseMetadata.INSTANCE.existsTable(con, "files")) {
            stmt.execute("CREATE CACHED TABLE files (" + "  id BIGINT GENERATED BY DEFAULT"
                         + " AS IDENTITY(START WITH 1, INCREMENT BY 1) PRIMARY KEY"
                         + ", filename VARCHAR_IGNORECASE(512) NOT NULL" + ", lastmodified BIGINT, thumbnail BINARY"
    ...
    private void createXmpTables(Connection con, Statement stmt) throws SQLException {
        ...
        if (!DatabaseMetadata.INSTANCE.existsTable(con, "dc_subjects")) {
            stmt.execute("CREATE CACHED TABLE dc_subjects (" + " id BIGINT GENERATED BY DEFAULT"
                         + " AS IDENTITY(START WITH 1, INCREMENT BY 1) PRIMARY KEY"
        ...
        if (!DatabaseMetadata.INSTANCE.existsTable(con, "xmp")) {
            stmt.execute("CREATE CACHED TABLE xmp (" + "  id BIGINT GENERATED BY DEFAULT"
                         + " AS IDENTITY(START WITH 1, INCREMENT BY 1) PRIMARY KEY"
                         ...
                         + ", id_iptc4xmpcore_location BIGINT" + ", id_photoshop_authorsposition BIGINT"
                         + ", id_photoshop_captionwriter BIGINT" + ", id_photoshop_city BIGINT"
                         + ", id_photoshop_country BIGINT" + ", id_photoshop_credit BIGINT"
                         + ", photoshop_headline VARCHAR_IGNORECASE(256)"
                         + ", photoshop_instructions VARCHAR_IGNORECASE(256)" + ", id_photoshop_source BIGINT"
                         + ", id_photoshop_state BIGINT" + ", photoshop_transmissionReference VARCHAR_IGNORECASE(32)"
                         + ", rating BIGINT" + ", iptc4xmpcore_datecreated VARCHAR_IGNORECASE(10)"
                         ...
                         + " REFERENCES photoshop_authorspositions (id)" + " ON DELETE SET NULL"
        ...
        if (!DatabaseMetadata.INSTANCE.existsTable(con, "xmp_dc_subject")) {
            stmt.execute("CREATE CACHED TABLE xmp_dc_subject (id_xmp BIGINT" + ", id_dc_subject BIGINT"
    ...
    private void create1nTable(Connection con, Statement stmt, String tablename, String columnname, int length)
        ...
        if (!DatabaseMetadata.INSTANCE.existsTable(con, tablename)) {
            stmt.execute("CREATE CACHED TABLE " + tablename + " (id BIGINT GENERATED BY DEFAULT"
                         + " AS IDENTITY(START WITH 1, INCREMENT BY 1) PRIMARY KEY, " + columnname
                         + " VARCHAR_IGNORECASE(" + Integer.toString(length) + "))");
            stmt.execute("CREATE UNIQUE INDEX idx_" + tablename + "_id" + " ON " + tablename + " (id)");
            stmt.execute("CREATE UNIQUE INDEX idx_" + tablename + "_" + columnname + " ON " + tablename + " ("
                         + columnname + ")");
        }
    }
    ...
    private void createExifTables(Connection con, Statement stmt) throws SQLException {
        ...
        if (!DatabaseMetadata.INSTANCE.existsTable(con, "exif")) {
            stmt.execute("CREATE CACHED TABLE exif (id BIGINT GENERATED BY DEFAULT"
                         + " AS IDENTITY(START WITH 1, INCREMENT BY 1) PRIMARY KEY" + ", id_file BIGINT NOT NULL"
                         + ", id_exif_recording_equipment BIGINT"
                         + ", exif_date_time_original DATE, exif_focal_length REAL"
                         + ", exif_iso_speed_ratings SMALLINT" + ", id_exif_lens  BIGINT, FOREIGN KEY (id_file)"
                         + " REFERENCES files (id) ON DELETE CASCADE" + ", FOREIGN KEY (id_exif_recording_equipment)"
    ...
    private void createCollectionsTables(Connection con, Statement stmt) throws SQLException {
        if (!DatabaseMetadata.INSTANCE.existsTable(con, "collection_names")) {
            stmt.execute("CREATE CACHED TABLE collection_names (" + "  id BIGINT GENERATED BY DEFAULT"
                         + " AS IDENTITY(START WITH 1, INCREMENT BY 1) PRIMARY KEY"
        ...
        if (!DatabaseMetadata.INSTANCE.existsTable(con, "collections")) {
            stmt.execute("CREATE CACHED TABLE collections (" + "  id_collectionnname BIGINT, id_file BIGINT"
                         + ", sequence_number INTEGER" + ", FOREIGN KEY (id_collectionnname)"
    ...
    private void createSavedSearchesTables(Connection con, Statement stmt) throws SQLException {
        if (!DatabaseMetadata.INSTANCE.existsTable(con, "saved_searches")) {
            stmt.execute("CREATE CACHED TABLE saved_searches (" + "  id BIGINT GENERATED BY DEFAULT"
                         + " AS IDENTITY(START WITH 1, INCREMENT BY 1) PRIMARY KEY"
                         + ", name VARCHAR_IGNORECASE(125), custom_sql BINARY" + ", search_type SMALLINT);");
        ...
        if (!DatabaseMetadata.INSTANCE.existsTable(con, "saved_searches_panels")) {
            stmt.execute("CREATE CACHED TABLE saved_searches_panels (" + "  id_saved_search BIGINT"
                         ...
                         + ", value VARCHAR(256), bracket_right BOOLEAN" + ", FOREIGN KEY (id_saved_search)"
                         + " REFERENCES saved_searches (id) ON DELETE CASCADE" + ");");
            stmt.execute("CREATE INDEX idx_saved_searches_panels_id_saved_search"
        ...
        if (!DatabaseMetadata.INSTANCE.existsTable(con, "saved_searches_keywords")) {
            stmt.execute("CREATE CACHED TABLE saved_searches_keywords (" + "  id_saved_search BIGINT"
                         + ", keyword VARCHAR_IGNORECASE(64)" + ", FOREIGN KEY (id_saved_search)"
                         + " REFERENCES saved_searches (id) ON DELETE CASCADE" + ");");
            stmt.execute("CREATE INDEX idx_saved_searches_keywords_id_saved_search"
    ...
    private void createAutoScanDirectoriesTable(Connection con, Statement stmt) throws SQLException {
        if (!DatabaseMetadata.INSTANCE.existsTable(con, "autoscan_directories")) {
            stmt.execute("CREATE CACHED TABLE autoscan_directories (" + "directory VARCHAR_IGNORECASE(1024));");
            stmt.execute("CREATE UNIQUE INDEX idx_autoscan_directories_directory"
    ...
    private void createMetadataTemplateTable(Connection con, Statement stmt) throws SQLException {
        if (!DatabaseMetadata.INSTANCE.existsTable(con, "metadata_edit_templates")) {
            stmt.execute("CREATE CACHED TABLE metadata_edit_templates (" + "  name VARCHAR_IGNORECASE(256)"
                         + ", dcSubjects BINARY, dcTitle BINARY" + ", photoshopHeadline BINARY"
                         + ", dcDescription BINARY" + ", photoshopCaptionwriter BINARY"
                         + ", iptc4xmpcoreLocation BINARY" + ", dcRights BINARY, dcCreator BINARY"
                         + ", photoshopAuthorsposition BINARY" + ", photoshopCity BINARY, photoshopState BINARY"
                         + ", photoshopCountry BINARY" + ", photoshopTransmissionReference BINARY"
                         + ", photoshopInstructions BINARY" + ", photoshopCredit BINARY"
                         + ", photoshopSource BINARY, rating BINARY" + ", iptc4xmpcore_datecreated BINARY);");
    ...
    private void createFavoriteDirectoriesTable(Connection con, Statement stmt) throws SQLException {
        if (!DatabaseMetadata.INSTANCE.existsTable(con, "favorite_directories")) {
            stmt.execute("CREATE CACHED TABLE favorite_directories(" + "id BIGINT GENERATED BY DEFAULT"
                         + " AS IDENTITY(START WITH 1, INCREMENT BY 1)"
                         + " PRIMARY KEY, favorite_name VARCHAR_IGNORECASE(256)" + ", directory_name VARCHAR(512)"
                         + ", favorite_index INTEGER);");
            stmt.execute("CREATE UNIQUE INDEX idx_favorite_directories_favorite_name"
    ...
    private void createFileExcludePatternsTable(Connection con, Statement stmt) throws SQLException {
        if (!DatabaseMetadata.INSTANCE.existsTable(con, "file_exclude_patterns")) {
            stmt.execute("CREATE CACHED TABLE file_exclude_patterns (" + "pattern VARCHAR_IGNORECASE(256));");
    ...
    private void createProgramsTable(Connection con, Statement stmt) throws SQLException {
        if (!DatabaseMetadata.INSTANCE.existsTable(con, "programs")) {
            stmt.execute("CREATE CACHED TABLE programs (" + "  id BIGINT NOT NULL, action BOOLEAN"
                         + ", filename VARCHAR(512) NOT NULL" + ", alias VARCHAR_IGNORECASE(250) NOT NULL"
                         + ", parameters_before_filename BINARY" + ", parameters_after_filename BINARY"
                         + ", input_before_execute BOOLEAN" + ", input_before_execute_per_file BOOLEAN"
                         + ", single_file_processing BOOLEAN" + ", change_file BOOLEAN" + ", sequence_number INTEGER"
    ...
    private void createActionsAfterDbInsertionTable(Connection con, Statement stmt) throws SQLException {
        if (!DatabaseMetadata.INSTANCE.existsTable(con, "actions_after_db_insertion")) {
            stmt.execute("CREATE CACHED TABLE actions_after_db_insertion " + " (id_program BIGINT NOT NULL"
                         + ", action_order INTEGER);");
            stmt.execute("CREATE UNIQUE INDEX idx_actions_after_db_insertion_id_programs"
                         + " ON actions_after_db_insertion (id_program)");
            stmt.execute("CREATE INDEX idx_actions_after_db_insertion_action_order"
    ...
    private void createHierarchicalSubjectsTable(Connection con, Statement stmt) throws SQLException {
        if (!DatabaseMetadata.INSTANCE.existsTable(con, "hierarchical_subjects")) {
            stmt.execute("CREATE CACHED TABLE hierarchical_subjects (" + "  id BIGINT NOT NULL, id_parent BIGINT"
                         + ", subject VARCHAR_IGNORECASE(64) NOT NULL" + ", real BOOLEAN);");
    ...
    private void createAppTable(Connection con, Statement stmt) throws SQLException {
        if (!DatabaseMetadata.INSTANCE.existsTable(con, "application")) {
            stmt.execute("CREATE CACHED TABLE application (" + " key VARCHAR(128) PRIMARY KEY, value BINARY);");
        }
    }
    ...
    private void createSynonymsTable(Connection con, Statement stmt) throws SQLException {
        if (!DatabaseMetadata.INSTANCE.existsTable(con, "synonyms")) {
            stmt.execute("CREATE CACHED TABLE synonyms (" + " word VARCHAR(128), synonym VARCHAR(128)"
    ...
    private void createRenameTemplatesTable(Connection con, Statement stmt) throws SQLException {
        if (!DatabaseMetadata.INSTANCE.existsTable(con, "rename_templates")) {
            stmt.execute("CREATE CACHED TABLE rename_templates (" + "  id BIGINT GENERATED BY DEFAULT"
                         + " AS IDENTITY(START WITH 1, INCREMENT BY 1) PRIMARY KEY"
                         + ", name VARCHAR(128) NOT NULL, start_number INTEGER"
                         + ", step_width INTEGER, number_count INTEGER" + ", date_delimiter VARCHAR(5)"
                         + ", format_class_at_begin VARCHAR(512)" + ", delimiter_1 VARCHAR(25)"
                         + ", format_class_in_the_middle VARCHAR(512)" + ", delimiter_2 VARCHAR(25)"
                         + ", format_class_at_end VARCHAR(512)" + ", text_at_begin VARCHAR(512)"
                         + ", text_in_the_middle VARCHAR(512)" + ", text_at_end VARCHAR(512), UNIQUE(name));");
    ...
    private void createUserDefinedFileFiltersTable(Connection con, Statement stmt) throws SQLException {
        if (!DatabaseMetadata.INSTANCE.existsTable(con, "user_defined_file_filters")) {
            stmt.execute("CREATE CACHED TABLE user_defined_file_filters" + " (id BIGINT GENERATED BY DEFAULT"
                         + " AS IDENTITY(START WITH 1, INCREMENT BY 1) PRIMARY KEY" + ", is_not BOOLEAN, type INTEGER"
                         + ", name VARCHAR_IGNORECASE(45) NOT NULL"
                         + ", expression VARCHAR(128) NOT NULL, UNIQUE(name));");
            stmt.execute("CREATE UNIQUE INDEX idx_user_defined_file_filters_name"
