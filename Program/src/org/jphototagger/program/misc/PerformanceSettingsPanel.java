...
public final class PerformanceSettingsPanel extends PanelExt implements Persistence, HelpPageProvider {
    ...
    private void spinnerMaximumSecondsToTerminateExternalProgramsStateChanged(javax.swing.event.ChangeEvent evt) {
        if (listen) {
            persistMaximumSecondsToTerminateExternalPrograms();
        }
    }

    private void checkBoxScanForEmbeddedXmpActionPerformed(java.awt.event.ActionEvent evt) {
        if (listen) {
            persistScanForEmbeddedXmp();
        }
    }

    private void checkBoxSaveInputEarlyActionPerformed(java.awt.event.ActionEvent evt) {
        if (listen) {
            persistSaveInputEarly();
        }
    }

    private void checkBoxEnableAutocompleteActionPerformed(java.awt.event.ActionEvent evt) {
        if (listen) {
            persistEnableAutocomplete();
        }
    }

    private void checkBoxUpdateAutocompleteActionPerformed(java.awt.event.ActionEvent evt) {
        if (listen) {
            persistUpdateAutocomplete();
        }
    }

    private void checkBoxAutocompleteFastSearchIgnoreCaseActionPerformed(java.awt.event.ActionEvent evt) {
        if (listen) {
            persistAutocompleteFastSearchIgnoreCase();
        }
    }

    private void checkBoxAutoscanDirectoriesActionPerformed(java.awt.event.ActionEvent evt) {
        if (listen) {
