...
public final class Join {
    ...
    static {
        ...
        JOIN_FROM_FILES.put("exif_recording_equipment",
                " \\JOIN_TYPE\\ JOIN exif ON files.id = exif.id_file"
                + " INNER JOIN exif_recording_equipment ON exif.id_exif_recording_equipment = exif_recording_equipment.id");
        JOIN_FROM_FILES.put("exif_lenses",
                " \\JOIN_TYPE\\ JOIN exif ON files.id = exif.id_file"
                + " INNER JOIN exif_lenses ON exif.id_exif_lens = exif_lenses.id");
        JOIN_FROM_FILES.put("dc_creators",
                " \\JOIN_TYPE\\ JOIN xmp ON files.id = xmp.id_file"
                + " INNER JOIN dc_creators ON xmp.id_dc_creator = dc_creators.id");
        JOIN_FROM_FILES.put("dc_rights",
                " \\JOIN_TYPE\\ JOIN xmp ON files.id = xmp.id_file"
                + " INNER JOIN dc_rights ON xmp.id_dc_rights = dc_rights.id");
        JOIN_FROM_FILES.put("iptc4xmpcore_locations",
                " \\JOIN_TYPE\\ JOIN xmp ON files.id = xmp.id_file"
                + " INNER JOIN iptc4xmpcore_locations ON xmp.id_iptc4xmpcore_location = iptc4xmpcore_locations.id");
        JOIN_FROM_FILES.put("photoshop_authorspositions",
                " \\JOIN_TYPE\\ JOIN xmp ON files.id = xmp.id_file"
                + " INNER JOIN photoshop_authorspositions ON xmp.id_photoshop_authorsposition = photoshop_authorspositions.id");
        JOIN_FROM_FILES.put("photoshop_captionwriters",
                " \\JOIN_TYPE\\ JOIN xmp ON files.id = xmp.id_file"
                + " INNER JOIN photoshop_captionwriters ON xmp.id_photoshop_captionwriter = photoshop_captionwriters.id");
        JOIN_FROM_FILES.put("photoshop_cities",
                " \\JOIN_TYPE\\ JOIN xmp ON files.id = xmp.id_file"
                + " INNER JOIN photoshop_cities ON xmp.id_photoshop_city = photoshop_cities.id");
        JOIN_FROM_FILES.put("photoshop_countries",
                " \\JOIN_TYPE\\ JOIN xmp ON files.id = xmp.id_file"
                + " INNER JOIN photoshop_countries ON xmp.id_photoshop_country = photoshop_countries.id");
        JOIN_FROM_FILES.put("photoshop_credits",
                " \\JOIN_TYPE\\ JOIN xmp ON files.id = xmp.id_file"
                + " INNER JOIN photoshop_credits ON xmp.id_photoshop_credit = photoshop_credits.id");
        JOIN_FROM_FILES.put("photoshop_sources",
                " \\JOIN_TYPE\\ JOIN xmp ON files.id = xmp.id_file"
                + " INNER JOIN photoshop_sources ON xmp.id_photoshop_source = photoshop_sources.id");
        JOIN_FROM_FILES.put("photoshop_states",
                " \\JOIN_TYPE\\ JOIN xmp ON files.id = xmp.id_file"
                + " INNER JOIN photoshop_states ON xmp.id_photoshop_state = photoshop_states.id");
        ...
        DELETE_SQL_OF.put("iptc4xmpcore_locations",
        ...
        DELETE_SQL_OF.put("photoshop_captionwriters",
    ...
    public static String getUnjoinedFilesSqlWhere(String tablename) {
        ...
        return "WHERE files.id NOT IN"
                + " (" + "SELECT files.id FROM files INNER JOIN " + tablename + " ON " + tablename + ".id_file = files.id)";
    }
    ...
    public static String removeMultipleJoinsToFiles(String sql, Type type) {
        String xmpToRemove = " \\JOIN_TYPE\\ JOIN xmp ON files.id = xmp.id_file".replace("\\JOIN_TYPE\\", type.string);
        String exifToRemove = " \\JOIN_TYPE\\ JOIN exif ON files.id = exif.id_file".replace("\\JOIN_TYPE\\", type.string);
        String newSql = removeMultipleJoinsToFiles(sql, xmpToRemove);
        return removeMultipleJoinsToFiles(newSql, exifToRemove);
}

    private static String removeMultipleJoinsToFiles(String sql, String toRemove) {
        String newSql = sql;
        while (StringUtil.getSubstringCount(newSql, toRemove) > 1) {
            newSql = StringUtil.removeLast(newSql, toRemove);
        }
        return newSql;
    }
}
